...
export const ELEMENTS: Record<string, ElementDef> = {
  ...
  // STORAGE
  battery_small: { id: 'battery_small', name: 'Battery (50kWh)', cost: 250, type: 'store', baseValue: 50, icon: '🔋', description: 'Small buffer. Degrades over time.' },
  battery_medium: { id: 'battery_medium', name: 'Battery (200kWh)', cost: 900, type: 'store', baseValue: 200, icon: '🔋', description: 'Medium buffer. Degrades over time.', width: 1, height: 2 },
  battery_large: { id: 'battery_large', name: 'Battery (500kWh)', cost: 2000, type: 'store', baseValue: 500, icon: '🔋', description: 'Large buffer. Degrades over time.', width: 2, height: 2 },
...
export const EVENTS: GameEvent[] = [
  {
    ...
    options: [
      { 
        label: 'Patch it up ($100)', 
        effect: (s) => ({ credits: s.credits - 100 }), 
        outcomeText: 'Cheap fix applied. Efficiency unaffected.' 
      },
      { 
        label: 'Full Upgrade ($600)', 
        effect: (s) => ({ 
           credits: s.credits - 600, 
           score: s.score + 50,
        }), 
  ...
  },
  {
    ...
    options: [
      { 
        label: 'Awareness Campaign ($100)', 
        effect: (s) => ({ credits: s.credits - 100, score: s.score + 30 }), 
  ...
  },
  {
    ...
    options: [
      { 
        label: 'Inspect foundations ($150)', 
        effect: (s) => ({ credits: s.credits - 150 }), 
    ...
    options: [
      {
        ...
        effect: (s) => {
          // If actual battery charge is sufficient, reward score.
          // Otherwise, apply huge penalty.
          const charge = s.batteryCharge || 0;
          if (charge > 150) {
             return { score: s.score + 150 }; 
          } else {
             return { credits: s.credits - 400, score: s.score - 50 };
          }
        },
        outcomeText: (s) => {
           ...
           if (charge > 150) {
               ...
               return `❌ FAILURE! Stored Charge (${Math.round(charge)}kWh) too low (Need 150kWh).\n\n💸 PENALTY: Emergency Repairs (-$400).`;
           }
        }
      }
    ]
